...
primer_test<-mutate_spm(input_sequence, prefix="TT", restriction_enzyme = recognition_site_bbsi, suffix = "AA", vector=c("CTCA", "CTCG"), replacements = mutations, binding_min_length=4 ,primer_length=9, target_temp=60, cuf=cuf)
...
primer_test_lc<-mutate_spm(input_sequence, prefix=str_to_lower("TT"), restriction_enzyme = str_to_lower(recognition_site_bbsi), suffix = str_to_lower("AA"), vector=str_to_lower(c("CTCA", "CTCG")), replacements = mutations, binding_min_length=4 ,primer_length=9, target_temp=60, cuf=cuf)
